...
from src.agent.nodes import intent_recognize_node, get_user_info, get_band_info_by_idcard, check_band_info, \
    router_after_get_user_info_node, router_after_get_band_info_by_idcard, get_fault_code, router_after_get_fault_code
...
def create_graph(llm_chat, checkpointer)-> StateGraph:
    ...
    workflow.add_node("send_orders", partial(get_fault_code, llm_chat=llm_chat))
    # 添加从起始到代理的边
    workflow.add_edge(START, "intent_recognize_node")
    workflow.add_edge("intent_recognize_node","get_user_info")
    workflow.add_conditional_edges("get_user_info", router_after_get_user_info_node)
    workflow.add_conditional_edges("get_band_info_by_idcard", router_after_get_band_info_by_idcard)
    workflow.add_edge("check_band_info", "get_fault_code")
    workflow.add_conditional_edges("get_fault_code", router_after_get_fault_code)
    workflow.add_edge("send_orders", END)
